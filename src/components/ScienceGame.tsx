...
import { FlaskConical, Star, Trophy, ArrowRight, Sparkles } from 'lucide-react';
...
export const ScienceGame: React.FC<ScienceGameProps> = ({ difficulty, lang, onActivityComplete }) => {
  ...
  const scienceQuestions = QUIZ_QUESTIONS.filter(
    (q) => q.category === 'science' && (q.difficulty === difficulty || difficulty === 'hard')
  );
  ...
  const currentQ = scienceQuestions[currentRound % (scienceQuestions.length || 1)];
  ...
  const handleNext = () => {
    ...
    const totalQ = scienceQuestions.length;
  ...
  const handleGameEnd = () => {
    ...
    const isPerfect = score === scienceQuestions.length;
    let baseStars = score * 12;
    let perfectBonus = isPerfect ? 25 : 0;
    const total = baseStars + perfectBonus;
    setStarsReward(total);

    // Science explorer badge on perfect score
    const earnedBadge = isPerfect ? 'science_explorer' : undefined;
    ...
    <div className="max-w-2xl mx-auto flex flex-col gap-4">
      ...
      <div className="w-full flex justify-between items-center bg-white p-3 border-4 border-slate-900 rounded-2xl shadow">
        <h3 className="font-bold text-lg text-slate-800 flex items-center gap-1.5">
          <FlaskConical className="w-5 h-5 text-kid-green animate-bounce" />
          {t.science} — {difficulty === 'easy' ? t.easy.split(' ')[0] : difficulty === 'medium' ? t.medium.split(' ')[0] : t.hard.split(' ')[0]}
        </h3>
        <VoiceReader text={getSpeakText()} lang={lang} label="Listen 🔊" />
      </div>

      {gameState === 'playing' ? (
        <div className="cartoon-card p-6 bg-white flex flex-col gap-6 animate-wiggle-short">
          <div className="flex flex-col gap-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">
              Science Quest {currentRound + 1} / {scienceQuestions.length}
            </span>
            ...
            {(lang === 'es' ? currentQ.optionsEs : lang === 'fr' ? currentQ.optionsFr : currentQ.optionsEn || []).map((opt) => {
              ...
              return (
                <button
                  ...
                  disabled={isAnswered}
                  id={`science-option-${opt.replace(/\s+/g, '-')}`}
        ...
        /* Science end screen rewards */
        <div className="cartoon-card p-6 bg-white text-center flex flex-col items-center gap-4">
          <Trophy className="w-16 h-16 text-yellow-500 animate-bounce" />
          <h2 className="text-2xl font-bold text-slate-800">
            {score === scienceQuestions.length ? t.perfect : t.greatJob}
          </h2>
          <p className="text-sm font-semibold text-slate-600">
            Wonderful exploring! You scored <span className="text-indigo-600 font-bold text-lg">{score}</span> out of{' '}
            <span className="font-bold text-lg">{scienceQuestions.length}</span> scientific discoveries!
          </p>
